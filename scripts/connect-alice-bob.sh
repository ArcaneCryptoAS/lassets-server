...
BOB_IP=`docker inspect --format '{{json .NetworkSettings}}' lna-bob | jq .Networks.server_default.IPAddress --raw-output`
BOB_PUBKEY=`./lnd-bob getinfo | jq --raw-output .identity_pubkey`
ALICE_IP=`docker inspect --format '{{json .NetworkSettings}}' lna-alice | jq .Networks.server_default.IPAddress --raw-output`
